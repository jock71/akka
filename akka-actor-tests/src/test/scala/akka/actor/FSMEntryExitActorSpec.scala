...
object FSMEntryExitActorSpec {
  ...
  class Latches(implicit system: ActorSystem) {
    ...
    val entryLatch = TestLatch()
    val exitLatch = TestLatch()
  }
  ...
  class Lock(code: String, timeout: FiniteDuration, latches: Latches)
    extends Actor with EntryExitFSM[LockState, CodeState] {
...
class FSMEntryExitActorSpec extends AkkaSpec(Map("akka.actor.debug.fsm" → true)) with ImplicitSender {
  import FSMEntryExitActorSpec._
  import FSM.`→`
  ...
  "An FSM Actor" must {
    ...
    "unlock the lock" in {
      ...
      Await.ready(entryLatch, timeout.duration)
      Await.ready(exitLatch, timeout.duration)
